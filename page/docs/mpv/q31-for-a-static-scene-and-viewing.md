> **Source question (Q31):** For a static scene and viewing by camera with only horizontal movement. Draw an image patch, which will be useful for a tracking using a gradient method (KLT tracker). Which properties should such image patch have to be suitable for tracking?
