...
The figure illustrates two practical tools. Panel (a) is a stylised learning-rate range test: sweep $\eta$ on a log axis from $10^{-6}$ to $10$ over a few mini-batches and plot the loss; three regimes appear — too small (flat loss, gray band), sweet spot (steep descent, green band), too large (divergent, red band). The recommended rate is roughly one order of magnitude below the divergence point. Panel (b) plots the four most common schedules on a log-$\eta$ axis: step decay (×0.1 every 30 epochs), exponential decay, cosine annealing (smooth half-cosine), and warmup → cosine (linear ramp-up for the first few epochs then cosine).

![Learning-rate range test and common schedules](../figures/q_0042_lr.png)
