...
The figure illustrates the effect. Panel (a) plots $f(u)$ for several exponents: $\alpha = 1$ is the linear baseline, while $\alpha = 2$, $3$, $5$ progressively flatten the response near $u = 0$ (noise) and steepen it near $|u| = 1$ (genuine matches). Panel (b) shows the consequence on aggregate scores when the same set of matches (200 weak + 4 strong) is summed under each $\alpha$: as $\alpha$ grows, the contribution of the 4 strong matches comes to dominate the 200 weak ones — the strong-to-weak ratio S:W rises from ~0.1 at $\alpha=1$ to above 5 at $\alpha=5$. This is the mechanism by which SMK closes much of the BoW–VLAD gap while keeping the inverted file.

![SMK: selective non-linear gating of residual dot products](../figures/q_0018_smk.png)
